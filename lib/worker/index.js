...
Worker.prototype.createWriteStream = function (callback) {
  ...
  that.channel.assertExchange(that.name, 'direct', { durable: true }, function (errAssertExchange) {
    if (errAssertExchange) {
      return callback(errAssertExchange);
    }

    that.channel.assertQueue(that.name, { durable: true }, function (errAssertQueue) {
      if (errAssertQueue) {
...
Worker.prototype.createReadStream = function (callback) {
  ...
  that.channel.assertExchange(that.name, 'direct', { durable: true }, function (errAssertExchange) {
    ...
    that.channel.assertQueue(that.name, { durable: true }, function (errAssertQueue) {
      ...
      that.channel.bindQueue(that.name, that.name, '', {}, function (errBindQueue) {
        var readStream;
